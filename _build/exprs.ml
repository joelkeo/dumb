...
and 'a cexpr = (* compound expressions *)
  ...
  | CLPrim2 of prim2 * 'a immexpr * 'a aexpr * 'a
  ...
  | CLambda of string * string list * 'a aexpr * 'a
...
let atag (p : 'a aprogram) : tag aprogram =
  ...
  and helpC (c : 'a cexpr) : tag cexpr =
    ...
    | CLambda(name, args, body, _) ->
       let lam_tag = tag() in
       CLambda(name, args, helpA body, lam_tag)
...
let get_tag_I i = 
  match i with
    ...
    | ImmId(_, tag) -> tag
    | ImmNil(tag) -> tag
let get_tag_C c = (* compound expressions *)
  match c with 
    CIf(_, _, _, tag) -> tag
  | CPrim1(_, _, tag) -> tag
  | CPrim2(_, _, _, tag) -> tag
  | CLPrim2(_, _, _, tag) -> tag
  | CApp(_, _, _, tag) -> tag
  | CImmExpr(i) -> get_tag_I i
  | CTuple(_, tag) -> tag
  | CGetItem(_, _, tag) -> tag
  | CSetItem(_, _, _, tag) -> tag
  | CLambda(_, _, _, tag) -> tag
let get_tag_A a =
  match a with
  | ASeq(_, _, tag) -> tag
  | ALet(_, _, _, tag) -> tag
