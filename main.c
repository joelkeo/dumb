...
SNAKEVAL printStack(SNAKEVAL val, uint64_t* rsp, uint64_t* rbp, uint64_t args) {
  ...
  if (rsp > rbp) {
    ...
    for (uint64_t* cur = rsp; cur < STACK_BOTTOM + 3; cur++) {
      if (cur == STACK_BOTTOM) {
        printf("BOT %llx: %llx\t==>  old rbp\n", (uint64_t)cur, *cur); fflush(stdout);
      } else if (cur == rbp) {
        printf("RBP %llx: %llx\t==>  old rbp\n", (uint64_t)cur, *cur); fflush(stdout);
      } else if (cur == origRsp) {
        printf("    %llx: %llx\t==>  old rbp\n", (uint64_t)cur, *cur); fflush(stdout);
      } else if (cur == rbp + 1) {
        printf("    %llx: %llx\t==>  saved ret\n", (uint64_t)cur, *cur); fflush(stdout);
        rsp = rbp + 2;
        rbp = (uint64_t*)(*rbp);
      } else if (cur == STACK_BOTTOM + 2) {
        printf("    %llx: %llx\t==>  heap\n", (uint64_t)cur, *cur); fflush(stdout);
      } else {
        printf("    %llx: %llx\t==>  ", (uint64_t)cur, *cur); fflush(stdout);
...
void error(uint64_t code, SNAKEVAL val) {
  ...
  case ERR_GET_LOW_INDEX:
    fprintf(stderr, "Error: index too small to get, got %llu\n", (uint64_t)val);
    break;
  case ERR_GET_HIGH_INDEX:
    fprintf(stderr, "Error: index too large to get, got %llu\n", (uint64_t)val);
    break;
  case ERR_NIL_DEREF:
  ...
  case ERR_CALL_ARITY_ERR:
    ...
    break;
  default:
    fprintf(stderr, "Error: Unknown error code: %llu, val: ", code); printHelp(stderr, val);
  }
  fprintf(stderr, "\n%p ==> ", (uint64_t*)val);
...
uint64_t* try_gc_donothing(uint64_t* alloc_ptr, uint64_t bytes_needed, uint64_t* cur_frame, uint64_t* cur_stack_top) {
  return alloc_ptr;
}
...
uint64_t* try_gc(uint64_t* alloc_ptr, uint64_t bytes_needed, uint64_t* cur_frame, uint64_t* cur_stack_top) {
  ...
  // variables of interest in our gc_loops
  ...
  // +1 for now TODO see if thats right
  new_r15 = gc(STACK_BOTTOM, cur_frame, cur_stack_top, FROM_S, HEAP_END, new_r15);
  // printf("THE NEW HEAP AFTER UPDATES: \n");
  // printf("HAS R15 OF: %llu", *new_heap_end);
  ...
  if (bytes_needed / sizeof(uint64_t) > HEAP_SIZE) {
    fprintf(stderr, "Allocation error: needed %llu words, but the heap is only %llu words\n",
    ...
    fprintf(stderr, "Out of memory: needed %llu words, but only %llu remain after collection\n",
            bytes_needed / sizeof(uint64_t), (uint64_t) (HEAP_END - new_r15));
    fflush(stderr);
    if (new_heap != NULL) free(new_heap);
    exit(ERR_OUT_OF_MEMORY);
  } else {
